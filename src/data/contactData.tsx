export interface ContactInfo {
  id: string;
  icon: React.ReactNode;
  label: string;
  value: string;
  href?: string;
  description?: string;
}

export interface BusinessHours {
  day: string;
  hours: string;
  isOpen: boolean;
}

export interface LocationInfo {
  name: string;
  address: string;
  city: string;
  country: string;
  postalCode: string;
  coordinates: {
    lat: number;
    lng: number;
  };
  description: string;
}

export interface ContactFormField {
  id: string;
  name: string;
  label: string;
  type: 'text' | 'email' | 'tel' | 'textarea' | 'select';
  placeholder: string;
  required: boolean;
  options?: string[];
  validation?: {
    minLength?: number;
    maxLength?: number;
    pattern?: string;
  };
}

export interface ContactSection {
  title: string;
  subtitle: string;
  description: string;
  contactInfo: ContactInfo[];
  businessHours: BusinessHours[];
  location: LocationInfo;
  formFields: ContactFormField[];
  apiEndpoint: string;
}

export const contactData: ContactSection = {
  title: "Contáctanos",
  subtitle: "Estamos aquí para ayudarte con tu proyecto textil",
  description: "Nuestro equipo de expertos está listo para asesorarte y crear las etiquetas perfectas para tu marca. Contáctanos para una cotización personalizada.",
  
  contactInfo: [
    {
      id: "phone",
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
        </svg>
      ),
      label: "Teléfono Principal",
      value: "+57 302 741 3967",
      href: "tel:+573027413967",
      description: "Lunes a Viernes: 8:00 AM - 6:00 PM"
    },
    {
      id: "whatsapp",
      icon: (
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
        </svg>
      ),
      label: "WhatsApp Business",
      value: "+57 302 741 3967",
      href: "https://wa.me/+573027413967",
      description: "Respuesta inmediata 24/7"
    },
    {
      id: "email",
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
        </svg>
      ),
      label: "Email Comercial",
      value: "zititex@gmail.com",
      href: "mailto:zititex@gmail.com",
      description: "Cotizaciones y consultas comerciales"
    }
  ],

  businessHours: [
    { day: "Lun a Vie", hours: "8:00 AM - 6:00 PM", isOpen: true },
    { day: "Sábado", hours: "9:00 AM - 2:00 PM", isOpen: true },
    { day: "Domingo", hours: "Cerrado", isOpen: false }
  ],

  location: {
    name: "Zititex - Planta de Producción",
    address: "Cl 19 sur 12g 45",
    city: "Bogotá",
    country: "Colombia",
    postalCode: "110111",
    coordinates: {
      lat: 4.583804, // Coordenadas exactas de Cl 19 sur 12g 45, Bogotá
      lng: -74.1016785
    },
    description: "Nuestra moderna planta de producción cuenta con tecnología de vanguardia para la fabricación de etiquetas y marquillas premium."
  },

  formFields: [
    {
      id: "name",
      name: "name",
      label: "Nombre Completo",
      type: "text",
      placeholder: "Ingresa tu nombre completo",
      required: true,
      validation: {
        minLength: 2,
        maxLength: 100
      }
    },
    {
      id: "company",
      name: "company",
      label: "Empresa",
      type: "text",
      placeholder: "Nombre de tu empresa (opcional)",
      required: false,
      validation: {
        maxLength: 100
      }
    },
    {
      id: "email",
      name: "email",
      label: "Correo Electrónico",
      type: "email",
      placeholder: "tu@email.com",
      required: true,
      validation: {
        pattern: "^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$"
      }
    },
    {
      id: "phone",
      name: "phone",
      label: "Teléfono",
      type: "tel",
      placeholder: "+57 300 123 4567",
      required: true,
      validation: {
        pattern: "^[+]?[0-9\\s\\-\\(\\)]{10,}$"
      }
    },
    {
      id: "productType",
      name: "productType",
      label: "Tipo de Producto",
      type: "select",
      placeholder: "Selecciona el tipo de producto",
      required: true,
      options: [
        "Etiquetas de Composición",
        "Marquillas de Marca",
        "Etiquetas Especiales",
        "Combinación de Productos",
        "Consulta General"
      ]
    },
    {
      id: "quantity",
      name: "quantity",
      label: "Cantidad Aproximada",
      type: "select",
      placeholder: "Selecciona la cantidad",
      required: false,
      options: [
        "100 - 500 unidades",
        "500 - 1,000 unidades",
        "1,000 - 5,000 unidades",
        "5,000 - 10,000 unidades",
        "Más de 10,000 unidades",
        "No estoy seguro"
      ]
    },
    {
      id: "message",
      name: "message",
      label: "Mensaje",
      type: "textarea",
      placeholder: "Cuéntanos sobre tu proyecto, especificaciones técnicas, fechas de entrega, o cualquier pregunta que tengas...",
      required: true,
      validation: {
        minLength: 10,
        maxLength: 1000
      }
    }
  ],
  // apiEndpoint: "/api/contact"
};

// Configuración del módulo de contacto
export const contactConfig = {
  showMap: true,
  showBusinessHours: true,
  showContactInfo: true,
  mapHeight: "400px",
  formValidation: true,
  successMessage: "¡Gracias por contactarnos! Te responderemos en las próximas 24 horas.",
  errorMessage: "Hubo un error al enviar tu mensaje. Por favor, intenta nuevamente o contáctanos directamente.",
};
